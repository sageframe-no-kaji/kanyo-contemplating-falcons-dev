...
def main():
    ...
    parser.add_argument("--extract-clips", action="store_true", help="Extract video clips around events")
    parser.add_argument("--dry-run", action="store_true", help="Show what clips would be extracted without extracting")
    parser.add_argument("video", nargs="?", default="data/samples/falcon_full_test.mov", help="Video file to analyze")
    ...
    if args.extract_clips or args.dry_run:
        ...
        for clip in clips:
            print(f"  - {clip.filename}: {clip.start_secs:.1f}s to {clip.end_secs:.1f}s ({clip.duration_secs:.1f}s)")
