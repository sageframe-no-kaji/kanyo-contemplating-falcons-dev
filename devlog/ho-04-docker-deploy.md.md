...
│   ├── docker-compose.yml              # Orchestration (persistent, git-tracked)
│   ├── .env                            # Paths (persistent, gitignored)
│   ├── Dockerfile                      # Image def (persistent, git-tracked)
│   ├── .dockerignore
│   ├── .gitignore
│   ├── src/                            # Code (persistent, git-tracked)
│   │   └── kanyo/
│   ├── models/                         # YOLO (persistent, large)
│   │   └── yolov8n.pt
│   ├── requirements.txt
│   └── web/                            # Admin UI
│       ├── Dockerfile
...
│       ├── requirements.txt
│       ├── templates/
│       ├── config.json                 # Stream registry (persistent, gitignored)
│       ├── users.db                    # Accounts (persistent, gitignored)
│       └── settings.yaml               # Preferences (persistent, gitignored)
│
├── kanyo-harvard/                      # ZFS: rpool/sage/kanyo/harvard
│   ├── clips/                          # Video clips (persistent, large)
│   │   └── 2025-12-18/
│   │       ├── falcon_143025_arrival.mp4
│   │       └── falcon_143025_arrival.jpg
│   ├── logs/                           # Logs (persistent, rotated)
│   │   └── kanyo.log
│   └── config.yaml                     # Stream config (persistent)
│
...
# Download YOLO model during build (baked into image)
...
RUN mkdir -p /app/clips /app/logs
...
# Run the detection monitor
CMD ["python", "-m", "kanyo.detection.realtime_monitor"]
```
...
services:
  # Stream 1: Harvard Falcon Cam
  harvard:
    build: .
    image: kanyo:latest
    container_name: kanyo-harvard
    volumes:
      # Bind mount ONLY persistent data from ZFS
      - ${KANYO_HARVARD_DATA:-./data/harvard}/clips:/app/clips
      - ${KANYO_HARVARD_DATA:-./data/harvard}/logs:/app/logs
      - ${KANYO_HARVARD_DATA:-./data/harvard}/config.yaml:/app/config.yaml:ro
      # Code is IN the image, not bind mounted
    environment:
      - PYTHONUNBUFFERED=1
  ...
  # Stream 2: NSW Falcon Cam
  nsw:
    build: .
    image: kanyo:latest
    container_name: kanyo-nsw
    volumes:
      - ${KANYO_NSW_DATA:-./data/nsw}/clips:/app/clips
      - ${KANYO_NSW_DATA:-./data/nsw}/logs:/app/logs
      - ${KANYO_NSW_DATA:-./data/nsw}/config.yaml:/app/config.yaml:ro
    environment:
      - PYTHONUNBUFFERED=1
    restart: unless-stopped
    logging:
  ...
  # Stream 3: Osprey Cam (example - commented out)
  # Uncomment when ready to add third stream
  # osprey:
  #   build: .
  #   image: kanyo:latest
  #   container_name: kanyo-osprey
  #   volumes:
  #     - ${KANYO_OSPREY_DATA:-./data/osprey}/clips:/app/clips
  #     - ${KANYO_OSPREY_DATA:-./data/osprey}/logs:/app/logs
  #     - ${KANYO_OSPREY_DATA:-./data/osprey}/config.yaml:/app/config.yaml:ro
  #   environment:
  #     - PYTHONUNBUFFERED=1
  #   restart: unless-stopped
  #   logging:
...
**Create `.env.example`:**
```bash
# Kanyo Docker Environment Configuration
# Copy this to .env and customize for your setup

# ============================================================================
# TIER 1: Quick Start (Relative Paths)
# ============================================================================
# Use this for simple deployments, testing, or Portainer
# Data will be stored in ./data/ relative to docker-compose.yml
# Uncomment these to use Tier 1:

# KANYO_HARVARD_DATA=./data/harvard
# KANYO_NSW_DATA=./data/nsw
# KANYO_OSPREY_DATA=./data/osprey

# ============================================================================
# TIER 2: Production (Absolute Paths with ZFS)
# ============================================================================
# Use this for production deployments with ZFS datasets
# Provides snapshots, compression, quotas, and proper governance
# Uncomment and customize these for your ZFS layout:
...
# ============================================================================
# ZFS Setup (for Tier 2)
# ============================================================================
# Before deploying, create ZFS datasets:
#
...
#
# Optional: Set compression and quotas
...
#
# Setup automatic snapshots (daily example):
...
**Each stream needs its own config.yaml in its ZFS-backed directory.**
...
rsync -av \
  ...
  atmarcus@kanyo:/opt/services/kanyo-admin/

# Transfer configs to their ZFS-backed locations
scp data/harvard/config.yaml atmarcus@kanyo:/opt/services/kanyo-harvard/
scp data/nsw/config.yaml atmarcus@kanyo:/opt/services/kanyo-nsw/
```
